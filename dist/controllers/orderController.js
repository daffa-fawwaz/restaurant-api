...
export const createOrder = async (req, res) => {
    try {
        ...
        if (unavailableMenu) {
            return error(res, 409, `Menu "${unavailableMenu.name}" is unavailable`, "MENU_UNAVAILABLE");
        }
        let subtotal = 0;
...
export const changeStatusOrder = async (req, res) => {
    try {
        ...
        // RESPONSE API
        // ==========================================
        return success(res, 200, "Update status success", updatedOrder);
    }
    catch (err) {
...
export const payOrder = async (req, res) => {
    try {
        ...
        if (Number.isNaN(receive) || receive <= 0) {
            return error(res, 400, "Amount received must be greater than 0", "MUST_GREATER_THEN_0");
        }
        ...
        if (order.status !== "SERVED") {
            return error(res, 400, "Order can only be paid when status is SERVED", "ORDER_NOT_SERVED");
        }
        const total = Number(order.total);
        if (receive < total) {
            return error(res, 400, `Insufficient payment. Total payment is ${total}`, "UANG_KURANG");
        }
...
export const getOrderById = async (req, res) => {
    try {
        ...
        return success(res, 200, "Success get order", formatOrderResponse(order));
    }
    catch (err) {
