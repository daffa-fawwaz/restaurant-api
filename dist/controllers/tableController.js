...
export const createTable = async (req, res) => {
    try {
        ...
        if (existTable) {
            return error(res, 400, "Table number already exists", "TABLE_ALREADY_EXISTS");
        }
...
export const toogleTableStatus = async (req, res) => {
    try {
        const { id } = req.params;
...
export const updateTable = async (req, res) => {
    try {
        ...
        if (existNumber) {
            return error(res, 409, "Table number already exists", "TABLE_NUMBER_ALREADY_EXISTS");
        }
        const updatedTable = await prisma.table.update({
            where: {
                id,
